...
os.environ["WANDB_NOTEBOOK_NAME"] = 'train_with_function_v2.py'
...
df = pd.read_hdf(datafolder + "prot_func_with_embedding.h5")
...
def preprocess_data(examples, max_length):
    ...
    # Ensure that labels are set up as a float tensor with shape (batch_size, 320)
    # encoding["labels"] = torch.tensor(examples["total_embedding"]).float()  # Adjust based on your data structure
    columns = ["e_" + str(i) for i in range(EMB_FEATURES)]
    encoding['labels'] = torch.tensor([examples[col] for col in columns]).float().T
    return encoding
...
model = EsmForSequenceClassification.from_pretrained(modelname, problem_type = 'regression',num_labels = EMB_FEATURES)
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        labels = inputs.get("labels").float()  # Ensure labels are float for regression
        outputs = model(**inputs)
        logits = outputs.logits  # Adjust if your model structure encapsulates logits in a different way
        loss = loss_fct(logits, labels)
        return (loss, outputs) if return_outputs else loss
